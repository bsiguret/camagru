...
function add_like($uid, $img, $type) {
  ...
  try {
      ...
      $query= $dbh->prepare("INSERT INTO `like`(`user_id`, `image_id`, type) SELECT :user_id, id, :type FROM `image` WHERE img=:img");
      $query->execute(array(':user_id' => $uid, ':img' => $img, ':type' => $type));
      return (0);
